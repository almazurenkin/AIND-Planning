...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set()) # Adding a new action layer            

        # Checking that ´level´ is right
        if len(self.a_levels) - 1 != level:
            raise ValueError('Value of ´level´ is incorrect!')

        for action in self.all_actions:
            # Create candidate action node object from an action: Action
            candidate = PgNode_a(action)
            # Check that all preconditions of the candidate action node are fulfilled in a connected state level...
            if candidate.prenodes.issubset(self.s_levels[level]):
                # ...and if so, add it to the action level we are creating, ...
                self.a_levels[level].add(candidate)
                # ...and connect it to all its precondition nodes in state level [level]                
                for precond_node in candidate.prenodes:
                    precond_node.children.add(candidate)
                    candidate.parents.add(precond_node)
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set()) # Adding a new state layer 

        # Checking that ´level´ is right
        if len(self.s_levels) - 1 != level:
            raise ValueError('Value of ´level´ is incorrect!')

        # Add effects of all actions from previous action layer to the new state layer...
        for action_node in self.a_levels[level - 1]:
            for effect_node in action_node.effnodes:
                self.s_levels[level].add(effect_node)
                # ...and ensure right connections between nodes in these layers
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Finding set of "conflicts" where one action negates an effect of the other
        conflicts = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) | \
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Finding set of "conflicts" where precondition of one action negates effects of the other
        # Taking into account both positive and negative effects!
        conflicts = set(node_a1.action.effect_add) & set(node_a2.action.precond_neg) | \
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Using PgNode.is_mutex method to test whether two nodes are mutex:
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return (node_s1.symbol == node_s2.symbol) & (node_s1.is_pos ^ node_s2.is_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        exclusive with ALL of the actions that could achieve the second literal
        node.
        ...
        for node_a1 in node_s1.parents:
            for node_a2 in node_s2.parents:
                # If at least two parent actions (that rtesult in the states we test) aren't mutex, 
                # we conclude no mutex:
                if not node_a1.is_mutex(node_a2):
                    return False
        # If ALL actions (that rtesult in the states we test) are mutex,
        # we conclude Inconsistent Support Mutex:        
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        # Level cost is the number of a state level in which our goal appears first
        for goal_clause in self.problem.goal:
            # Convert goal_node: Expr to a PGNode_s object to allow using __eq__ method for comparison
            goal_node = PgNode_s(goal_clause, True)
            for s_level_index in range(len(self.s_levels)):
                if goal_node in self.s_levels[s_level_index]:
                    level_sum += s_level_index
                    break
